...
export const createOrder = async (data,access_token) => {
  const res = await axiosJWT.post(`${process.env.REACT_APP_API_URL}/order/create-order/${data.user}`, data, {
      headers: {
...
export const getAllUserOrders = async (id,access_token) => {
  const res = await axiosJWT.get(`${process.env.REACT_APP_API_URL}/order/get-all-user-orders/${id}`, {
      headers: {
          token: `Bearer ${access_token}`,
      }
  })
