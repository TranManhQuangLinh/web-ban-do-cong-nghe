...
export const getAllProducts = async (search, limit) => {
    let res = {}
    if (search?.length > 0) {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-all-products?filter=name&filter=${search}&limit=${limit}`)
    } else {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-all-products?limit=${limit}`)
    }
    return res.data
}
...
export const deleteManyProducts = async (data, access_token,) => {
    const res = await axiosJWT.post(`${process.env.REACT_APP_API_URL}/product/delete-many-products`, data, {
        headers: {
            token: `Bearer ${access_token}`,
        }
    })
